...
<div>
  ...
  <el-table :data="list" border fit highlight-current-row style="width: 100%"  >
   <el-table-column  type="selection" align="center"  />
    <el-table-column min-width="100px" align="center" label="编号" prop="customer_stage">
    ...
    >
      <template slot-scope="scope">
        <span class="link-type" @click="operation('complaintDetail')">{{ scope.row.customer_name }}</span>
      </template>
    </el-table-column>

    <el-table-column min-width="180px" align="center" label="客户名称" prop="customer_state">
      <template slot-scope="scope">
        <span class="link-type" @click="operation('relation')">{{ scope.row.customer_state}}</span>
      </template>
    </el-table-column>

    <el-table-column min-width="100px" align="center" label="投诉内容" prop="customer_stage">
      <template slot-scope="scope">
        <span class="link-type">{{ scope.row.customer_stage }}</span>
      </template>
    </el-table-column>

    <el-table-column min-width="100px" align="center" label="紧急程度" prop="customer_level">
      <template slot-scope="scope">
        <span class="link-type">{{ scope.row.customer_level }}</span>
      </template>
    </el-table-column>

    <el-table-column min-width="100px" label="录入人" prop="customer_origin">
       <template slot-scope="scope">
        <span class="link-type">{{ scope.row.customer_origin }}</span>
      </template>
    </el-table-column>
    <el-table-column min-width="100px" label="录入时间" prop="customer_origin">
       <template slot-scope="scope">
        <span class="link-type">{{ scope.row.customer_origin }}</span>
      </template>
    </el-table-column>
    <el-table-column min-width="100px" label="处理状态" prop="customer_origin">
       <template slot-scope="scope">
        <span class="link-type">{{ scope.row.customer_origin }}</span>
      </template>
    </el-table-column>
    <el-table-column min-width="100px" label="处理人" prop="customer_origin">
       <template slot-scope="scope">
        <span class="link-type">{{ scope.row.customer_origin }}</span>
      </template>
    </el-table-column>
  ...
  <el-dialog :title="textMap[operation_type]" :visible.sync="showDialogFlag"  width="60%">
    ...
    <complaintDetail v-if="operation_type=='complaintDetail'"></complaintDetail>
